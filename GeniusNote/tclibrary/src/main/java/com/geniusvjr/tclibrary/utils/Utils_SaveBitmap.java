...
public class Utils_SaveBitmap {
	...
	public boolean saveBitmapAsJpg(Bitmap bitmap, String filePath){
		Debug.Log(this.getClass().getName() + "#saveBitmapAsJpg()", "filePath = " + filePath);
		...
		try {
			...
			Debug.Log(this.getClass().getName(), "保存图片时出错！code 1");
			e.printStackTrace();
			return false;
		}
		...
		try {
			...
			e1.printStackTrace();
			Debug.Log(this.getClass().getName(), "保存图片时出错！无法flush。code 2");
			return false;
		}
		...
		try {			
			...
			Debug.Log(this.getClass().getName(), "保存图片时出错！无法关闭fos。code 2");
			e.printStackTrace();
		}
		return true;
	}
